...
//	ST_test();  // internal consistency check
...
#endif
	{
		...
		if (!EM_recorded_any_errors()) {
			...
			// do another em recorded any errors if statment here - should be fine if the function returns void, so long
			// as errors are properly called for
			...
			if (! EM_recorded_any_errors()) {
				...
				EM_debug("Thank you please come again!");
				if (! EM_recorded_any_errors()) {
					// hera data may go here
					cout << code;
					return 0; // no errors
				}
			}
